...
public:
    static float Noise2D(float x, float z) {
        // Реализация базового когерентного шума для сетки
        ...
        // Псевдослучайная хэш-таблица (перестановка) встроенная для простоты
        static const int p[512] = {
            ...
            // Дублируем массив для предотвращения выхода за границы
            151,160,137,91,90,15,131,13,201,95,96,53,194,233,7,225,140,36,103,30,69,142,8,99,37,240,21,10,23,
            190, 6,148,247,120,234,75,0,26,197,62,94,252,219,203,117,35,11,32,57,177,33,88,237,149,56,87,174,
            20,125,136,171,168, 68,175,74,165,71,134,139,48,27,166,77,146,158,231,83,111,229,122,60,211,133,
            230,220,105,92,41,55,46,245,40,244,102,143,54, 65,25,63,161, 1,216,80,73,209,76,132,187,208, 89,
        ...
        return Lerp(v, Lerp(u, Grad(p[A], x, z),
                               Grad(p[B], x - 1, z)),
                       Lerp(u, Grad(p[A + 1], x, z - 1),
                               Grad(p[B + 1], x - 1, z - 1)));
    }
};